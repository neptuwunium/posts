...
This has the big advantage of only really using 4 (or 8) bytes of memory, as opposed to an integer for each button state and the accompanying loop every button to check if the sequence matches. We're also not pre-emptively checking if the input state is correct for this code, we simply only care for the correct sequence. This has an added benefit of being a little harder to notice if somene is hunting through game source code for debug or cheat input sequences.
